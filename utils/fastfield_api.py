...
def get_photo_bytes(filename: str, session_token: str,
    ...
    try:
        ...
        # Red inestable en campo: una foto que falla no puede tumbar la app.
        # El llamador la reporta y el usuario la sube a mano.
        return None
...
def download_submission_photos(photo_filenames: list[str],
    ...
    for fn in photo_filenames:
        ...
        if b is None:                       # un reintento: suele ser transitorio
            b = get_photo_bytes(fn, token, subscription_key)
        results.append(b)
        if b is None:
            errors.append(fn)
